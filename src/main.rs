...
fn main() {
    ...
    match &cli.command {
        None => { todo!(); }
        Some(cmd) => match cmd {
            Commands::Init { title: name } => { println!("Create new notebook in directory"); todo!() },
            Commands::Config { global } => { println!("Edit the config of the notebook"); todo!(); },
            Commands::Add { subcommand } => { 
                match subcommand {
                    AddCommand::Page{ title: name } => { println!("Add a new page to the current directory (either notebook or chapter)"); todo!(); },
                    AddCommand::Chapter { title } => { println!("Add a new chapter to the notebook"); todo!(); }
                }
            },
